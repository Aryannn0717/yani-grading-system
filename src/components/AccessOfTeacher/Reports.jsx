...
import { FileDown, CheckCircle2, XCircle, BarChart2, TrendingUp, TrendingDown } from "lucide-react"
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            {subject?.code} - {new Date().toLocaleDateString()}
          </Text>
        </View>
          ...
          {students.map((student) => {
            ...
            return (
              <View key={student.id} style={styles.tableRow}>
                ...
                <View style={styles.tableCol}>
                  <Text style={[styles.tableCell, performance.passed ? { color: "green" } : { color: "red" }]}>
            ...
            <View key={student.id} style={styles.tableRow}>
              ...
              {sessionDates.map((date) => (
                <View key={date} style={styles.tableCol}>
                  <Text style={styles.tableCell}>{attendanceBySession[date][student.id] || "N/A"}</Text>
                </View>
              ))}
            </View>
          ))}
...
export default function ReportsPage() {
  ...
  const calculateTermStats = (term) => {
    ...
    return {
      ...
      highest: Number.parseFloat(highest.toFixed(2)),
      lowest: Number.parseFloat(lowest.toFixed(2)),
    }
  }
  ...
  const createAttendanceDocument = () => {
    ...
    return (
      <PDFDownloadLink
        document={<AttendancePDF students={students} subject={subject} attendanceRecords={attendanceRecords} />}
        fileName={`${subject.code}_attendance.pdf`}
      >
        {({ loading }) => (
          <button
            className="flex items-center gap-2 border border-[#8697c4] px-5 py-2.5 rounded-lg hover:bg-[#ede8f5] transition-colors"
            disabled={loading}
          >
  ...
  const createGradebookDocument = () => {
    ...
    return (
      ...
      >
        {({ loading }) => (
          <button
            className="flex items-center gap-2 border border-[#8697c4] px-4 py-2 rounded-lg hover:bg-[#ede8f5] transition-colors text-sm"
            disabled={loading}
          >
            <FileDown className="h-4 w-4" />
            {loading ? "Generating..." : "Generate PDF"}
  ...
  return (
    <TeacherLayout title="Academic Reports">
      ...
      <div className="mb-8">
        ...
        <div className="bg-white rounded-lg border border-[#adbbda] shadow-md">
          <div className="p-5 border-b border-[#adbbda] bg-[#ede8f5]">
          ...
          <div className="p-5">
            <div className="space-y-3">
              ...
              <select
                id="subject"
                className="w-full px-4 py-2.5 border border-[#8697c4] rounded-lg bg-white focus:ring-2 focus:ring-[#adbbda]"
      ...
      {!isLoading && selectedSubject && (
        <div className="space-y-6">
          <div className="mb-5">
            <div className="flex border-b border-[#adbbda]">
              <button
                className={`py-3 px-5 text-sm font-medium ${
                  activeTab === "attendance"
                    ? "border-b-2 border-[#3d52a0] text-[#3d52a0]"
            ...
            <div className="bg-white rounded-lg border border-[#adbbda] shadow-md">
              ...
              <div className="p-5">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    className="flex items-center gap-2 bg-[#3d52a0] text-white px-5 py-2.5 rounded-lg hover:bg-[#7091e6] transition-colors"
              ...
              <div className="bg-white rounded-lg border border-[#adbbda] shadow-md">
                ...
                <div className="p-5">
                  {classStats ? (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                      <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm">
                            Avg: <span className="font-bold text-blue-600">{classStats.prelim?.average || "N/A"}</span>
                          </p>
                          <p className="text-sm">
                            High:{" "}
                            <span className="font-bold text-green-600">{classStats.prelim?.highest || "N/A"}</span>
                          </p>
                          <p className="text-sm">
                            Low: <span className="font-bold text-red-600">{classStats.prelim?.lowest || "N/A"}</span>
                          </p>
                        </div>
                      </div>

                      <div className="bg-[#ede8f5] p-4 rounded-lg border border-[#adbbda]">
                        <div className="flex items-center mb-2">
                          <BarChart2 className="h-5 w-5 text-[#7091e6] mr-2" />
                          <h4 className="text-sm font-medium text-[#3d52a0]">Midterm Stats</h4>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm">
                            Avg:{" "}
                            <span className="font-bold text-[#3d52a0]">{classStats.midterm?.average || "N/A"}</span>
                          </p>
                          <p className="text-sm">
                            High:{" "}
                            <span className="font-bold text-green-600">{classStats.midterm?.highest || "N/A"}</span>
                          </p>
                          <p className="text-sm">
                            Low: <span className="font-bold text-red-600">{classStats.midterm?.lowest || "N/A"}</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm">
                            Avg: <span className="font-bold text-amber-600">{classStats.final?.average || "N/A"}</span>
                          </p>
                          <p className="text-sm">
                            High: <span className="font-bold text-green-600">{classStats.final?.highest || "N/A"}</span>
                          </p>
                          <p className="text-sm">
                            Low: <span className="font-bold text-red-600">{classStats.final?.lowest || "N/A"}</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-green-50 p-4 rounded-lg border border-green-100">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm">
                            Pass Rate:{" "}
                            <span className="font-bold text-green-600">{classStats.passRate.toFixed(1)}%</span>
                          </p>
                          <p className="text-sm">
                            Total Students: <span className="font-bold text-gray-600">{students.length}</span>
                          </p>
                          <p className="text-sm">
                            Passing:{" "}
                            <span className="font-bold text-green-600">
                              {Math.round((students.length * classStats.passRate) / 100)}
                            </span>
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="text-center py-6 text-gray-500">No grade data available for this course</div>
                  )}
                </div>
              </div>

              <div className="bg-white rounded-lg border border-[#adbbda] shadow-md overflow-hidden">
                <div className="p-5 border-b border-[#adbbda] bg-[#ede8f5]">
                              ...
                              <div className="flex items-center">
                                {performance.passed ? (
                                  <>
                                    ...
                                    <span className="ml-2 text-sm text-green-600">Passed</span>
                                  </>
                                ) : (
                                  <>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50 text-right">
                  <div className="flex justify-between items-center">
                    <div className="text-sm text-gray-500">
                      Showing {students.filter((s) => calculateAcademicPerformance(s.id).cumulative.final > 0).length}{" "}
                      of {students.length} students with grades
                    </div>
                    <div className="flex space-x-2">
                      <button
                        className="flex items-center gap-2 bg-[#3d52a0] text-white px-4 py-2 rounded-lg hover:bg-[#7091e6] transition-colors text-sm"
