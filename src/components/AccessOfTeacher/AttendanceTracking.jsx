...
export default function AttendanceTracker() {
  ...
  const statusConfig = {
    present: {
      ...
      border: "border-l-4 border-l-[#3d52a0]",
      label: "Present"
    },
    absent: {
      icon: <X className="h-4 w-4" />,
      color: "bg-[#ede8f5] border-[#adbbda] text-[#3d52a0]",
      border: "border-l-4 border-l-[#8697c4]",
      label: "Absent"
    },
    late: {
      icon: <Clock className="h-4 w-4" />,
      color: "bg-[#ede8f5] border-[#adbbda] text-[#3d52a0]",
      border: "border-l-4 border-l-[#7091e6]",
      label: "Late"
    },
    excused: {
      icon: <AlertCircle className="h-4 w-4" />,
      color: "bg-[#ede8f5] border-[#adbbda] text-[#3d52a0]",
      border: "border-l-4 border-l-[#adbbda]",
      label: "Excused"
    }
  }
  ...
  return (
    <TeacherLayout title="Class Attendance">
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow-sm border border-[#adbbda]">
          <div className="p-6">
            <h1 className="text-2xl font-bold text-[#3d52a0] mb-6">Attendance Management</h1>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-[#8697c4] mb-2">Select Subject</label>
                <div className="relative">
                  <select
                    className="block w-full px-4 py-3 text-base border border-[#adbbda] rounded-lg shadow-sm focus:ring-[#7091e6] focus:border-[#7091e6]"
              ...
              <div>
                <label className="block text-sm font-medium text-[#8697c4] mb-2">Session Date</label>
                <div className="relative">
                  <button
                    className="flex items-center justify-between w-full px-4 py-3 text-left border border-[#adbbda] rounded-lg shadow-sm bg-white focus:ring-[#7091e6] focus:border-[#7091e6]"
                    onClick={() => setIsCalendarOpen(!isCalendarOpen)}
                  >
                    <div className="flex items-center">
                      <CalendarIcon className="mr-3 h-5 w-5 text-[#3d52a0]" />
                      <span>{format(selectedDate, "MMMM d, yyyy")}</span>
                    </div>
                    <svg className="h-5 w-5 text-[#8697c4]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                    </svg>
                  </button>
                  {isCalendarOpen && (
                    <div className="absolute z-10 mt-1 w-full bg-white border border-[#adbbda] rounded-lg shadow-lg p-4">
        ...
        {selectedSubject && (
          <div className="bg-white rounded-lg shadow-sm border border-[#adbbda] overflow-hidden">
            <div className="px-6 py-4 border-b border-[#adbbda] bg-[#ede8f5]">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-[#3d52a0]">Attendance Records</h2>
                  <p className="text-lg font-bold text-[#8697c4] mt-1">
                    {subjects.find(s => s.id === selectedSubject)?.name} • {format(selectedDate, "MMMM d, yyyy")}
                  </p>
                </div>
                <span className="mt-2 sm:mt-0 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-[#ede8f5] text-[#3d52a0]">
